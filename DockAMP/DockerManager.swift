...
class DockerManager: ObservableObject {
    ...
    private func createPHPConfig(_ config: ServerConfiguration) throws -> String {
        ...
        let phpIniPath = tempDir.appendingPathComponent("php_\(config.id).ini")
        
        try phpIni.write(to: phpIniPath, atomically: true, encoding: .utf8)
        
        return phpIniPath.path
    }
    ...
    private func createPHPFPMConfig(_ config: ServerConfiguration) throws -> String {
        ...
        """
        let tempDir = FileManager.default.temporaryDirectory
        let fpmConfPath = tempDir.appendingPathComponent("php_fpm_\(config.id).conf")
        try fpmConfig.write(to: fpmConfPath, atomically: true, encoding: .utf8)
        return fpmConfPath.path
    }
    ...
    private func createNginxConfig(_ config: ServerConfiguration) throws -> String {
        ...
        let tempDir = FileManager.default.temporaryDirectory
        let nginxConfPath = tempDir.appendingPathComponent("nginx_\(config.id).conf")

        try nginxConf.write(to: nginxConfPath, atomically: true, encoding: .utf8)

        return nginxConfPath.path
    }
    ...
    private func createNginxMainConfig(_ config: ServerConfiguration) throws -> String {
        ...
        let tempDir = FileManager.default.temporaryDirectory
        let nginxMainConfPath = tempDir.appendingPathComponent("nginx_main_\(config.id).conf")

        try nginxMainConf.write(to: nginxMainConfPath, atomically: true, encoding: .utf8)

        return nginxMainConfPath.path
    }
